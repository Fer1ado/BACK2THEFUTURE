...
export const vistaCheckout = async (req, res, next) => {

        try{
            const cid = req.params.cid
            const user = req.session
    
            const response = await ticketManager.generateTicket(cid, user)
           // console.log("🚀 ~ file: cart.controller.js:106 ~ purchaseCartById ~ response:", response);

            const tick = response.ticket
           
            
            const destroyCart = await MongoCartManager.deleteCart(cid)
            const assignCart = await MongoCartManager.createCart()
            req.session.user.cart = assignCart.cart._id.toString()

            if (destroyCart){
                res.render("checkout", {
                    code: tick.code,
                    date: tick.purchaseTimeStamp,
                    total: tick.amount,
                    cart: cid,
                    purchaser: tick.purchaser,
                    products: tick.cart,
                    id: tick.id,
                    unsold: response.unsold
                })
            }
            else{ res.json({status: "failed", message: "error al procesar la orden de compra intentalo nuevamente o contactate con nuestro equipo"})}
